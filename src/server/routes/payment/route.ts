...
export const paymentRoutes = new Hono()
  .post(
    ...
    async (c) => {
      ...
      if (!verifySignature(params, apiKey.secretKey, signature)) {
        return c.json(
          { status_code: 401, message: "Invalid signature" },
          401
        )
      }
  ...
  .get("/check-status/:tradeId", async (c) => {
    ...
    if (!order) {
      return c.json({ status_code: 10008, message: "Order not found" })
    }
...
async function getAmountPrecision(): Promise<number> {
  ...
  if (!setting) return 4
  const precision = parseInt(setting.value)
  return isNaN(precision) || precision < 2 || precision > 6 ? 4 : precision
}
